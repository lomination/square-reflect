    ...
    state match
      ...
      case _ =>
        throw UnsupportedOperationException(
          "Cannot get the next position of a non-`IsMoving` player with `.nextPosition` method."
        )

  /** Returns this with the given direction as `this.direction`. Also increments this player's number of move by 1. It
    * should be used only if this player's stats is `IsStopped`.
    */
  ...
  def forward: Player =
    state match
      case IsMoving(direction) =>
        setPositions(positions :+ positions.last.next(direction))
      case _ =>
